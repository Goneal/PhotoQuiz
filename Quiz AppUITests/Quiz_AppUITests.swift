...
final class Quiz_AppUITests: XCTestCase {
    ...
    func testNextGameButtonFunctionality() throws {
        ...
        // Test navigation to another specific game
        app.buttons.element(boundBy: 1).tap() // Tap the second game in the list
        XCTAssertTrue(app.staticTexts.element(matching: .any, identifier: "QuestionText").exists, "Second specific game should have started")

        // Verify back button functionality again
        backButton.tap()
        XCTAssertTrue(app.navigationBars["Quiz Games"].exists, "Should be back at the game index after second specific game")
    }
